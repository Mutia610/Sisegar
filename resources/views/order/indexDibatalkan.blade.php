      ...
      <div class="card card-primary">
        ...
        <div class="card-body table-responsive">
            ...
            @if(Request::get('keyword'))
              <a href="{{ route('pesananDibatalkan') }}" class="btn btn-success">Back</a>
            @endif

            <hr />
            <form method="get" action="{{ route('pesananDibatalkan')  }}">
              <div class="row">
                <div class="col-2">
                  <b>Search Name</b>
                </div>
            ...
            <table class="table table-bordered">
                ...
                <td>{{ $loop->iteration + ($order->perPage() * ($order->currentPage() - 1)  )  }}</td>
                    ...
                    <td>{{ $row->status }}</td>
                    <td>
                      <a href="{{ route('showDibatalkan',[$row->id]) }}" class="btn btn-warning btn-sm">Show</a>
                      </td>
                </tr>
                @endforeach
                </tbody>
